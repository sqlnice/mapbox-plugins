...
class GraticuleLayer {
    ...
    constructor(options: GraticuleLayerOptions) {
        ...
        extend(this, pick(options, ['showLabel', 'showTick', 'tickLength', 'showBorder', 'showGrid',
            "intervalUnit", "interval", 'bounds', "minZoom", "maxZoom", "labelStyle",
            "tickStyle", "gridStyle", "borderStyle", "labelFormatter"]));
        this._map = null;
        this._layerIDs = null;
        //存储图标ID数组
        this._iconIds = [];
        bindAll(['initLayersId', 'addTo', 'addLayers', 'createLayer', 'removeFromMap', 'setBounds', 'setInterval', 'update', 'toJSON'], this);
    }
    /***
     * @description 生成图标
     */
    ...
    addLayers() {
        ...
        let gridGeoJSON = EMPTY_GEOSJON;
        let tickGeoJSON = EMPTY_GEOSJON;
        let borderGeoJSON = EMPTY_GEOSJON;
        let labelGeoJSON = EMPTY_GEOSJON;
        if (!this._layerIDs) {
            this.initLayersId();
        }
    ...
    createTickLinesSource(lngLines, latLines, map, tickLen) {
        ...
        tickLen = tickLen || 5;
        let lineFeatures = [];
        const createTickLine = function (lnglat, offset, xAxis) {
    ...
    createLabelPointsSource(lngLines, latLines, map, tickLen) {
        ...
        const createPoint = function (lnglat, offset, xAxis, textAnchor) {
            let point = map.project(lnglat);
            let isLng = true;
            let rotate = 0;
            let num = 0;
    ...
    computeGrid() {
        ...
        if (intervalUnit == "d") {
            ...
            for (let n = minX; n <= maxX; n += interval) {
                lngLines.push([[n, south], [n, north]]);
            }
            for (let n = minY; n <= maxY; n += interval) {
                latLines.push([[west, n], [east, n]]);
            }
            ...
            for (let n = minX; n <= maxX; n += interval) {
                lngLines.push([[n / 60, south], [n / 60, north]]);
            }
            for (let n = minY; n <= maxY; n += interval) {
                latLines.push([[west, n / 60], [east, n / 60]]);
            }
        } else {
            throw ("不支持分隔单位");
        }
     ...
     * @param {String|Number} fontWeight 文字颜色
     * @returns {ImageData}
     */
    drawLabel(text, fontFamily = "sans-serif", fontSize = 16, color = "#000000", fontWeight = "normal") {
        let canvas = document.createElement("canvas");
        let len = text ? text.length : 0;
        let pixelRatio = browser.devicePixelRatio || 1;
